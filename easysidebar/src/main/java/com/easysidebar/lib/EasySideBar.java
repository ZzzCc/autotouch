...
public class EasySideBar extends View {
    ...
    private final static int DEFAULT_MAX_OFFSET = 32;
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        mIndexItemHeight = fontMetrics.bottom - fontMetrics.top + 16;
        mBarHeight = mIndexItems.length * mIndexItemHeight;

        while (mBarHeight >= mMaxHeight) {
            mTextSize--;
            mPaint.setTextSize(mTextSize);
        ...
        float areaLeft = (mSideBarPosition == POSITION_LEFT) ? 0 : (mMaxWidth - mBarWidth - getPaddingRight());
        float areaRight = (mSideBarPosition == POSITION_LEFT) ? (getPaddingLeft() + areaLeft + mBarWidth) : width;
        float areaTop = mMaxHeight / 2 - mBarHeight / 2;
        ...
        mFirstItemBaseLineY = (mMaxHeight / 2 - mIndexItems.length * mIndexItemHeight / 2)
